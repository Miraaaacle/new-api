...
func NotifyAdminsOfUserQuotaLow(userId int, userEmail string, userQuota int) {
	...
	// 只有存在至少一个启用了阈值的管理员时才查询用户名
	needNotify := false
	for _, admin := range admins {
		if admin.GetSetting().AdminUserQuotaNotifyThreshold > 0 {
			needNotify = true
			break
		}
	}
	...
	for _, admin := range admins {
		...
		if adminSetting.AdminUserQuotaNotifyThreshold <= 0 {
			continue
		}
		if userQuota >= int(adminSetting.AdminUserQuotaNotifyThreshold) {
			continue
		}
		subject := "用户额度预警"
		content := "用户 {{value}}（#{{value}}）当前余额 {{value}}，已低于预警阈值 {{value}}，请关注。"
		values := []interface{}{username, userId, userQuota, int(adminSetting.AdminUserQuotaNotifyThreshold)}
		notification := dto.NewNotify(dto.NotifyTypeAdminQuotaWarning, subject, content, values)
